//promise는 기본적으로 자바스크립트 비동기처리에 사용되는 객체
...
//promise의 인자로 함수를 전달받는데 이것을 executor라고 한다
//객체 생성과 동시에 즉시 실행 됨
//executor의 인자인 resolve는 비동기 작업이 성공했을 때 호출하는 함수
//executor의 인자인 reject는 비동기 작업이 실패했을 때 호출하는 함수
function getData() {
...
promise
  .then((data) => {
    //결과값을 매개변수로 받아옴
    //then은 promise가 이행(fulfilled)되었을 때 호춯되는 콜백함수
    //   console.log(data);
    const name = data.name;
    console.log(`이름은 ${name}입니다.`);
  })
  .catch((error) => {
    //catch는 promise가 거부(rejected)되었을 때 호출되는 콜백함수
    // console.log(error);
    console.log("예러 처리");
  })
  .finally(() => {

...
//게시판 목록 조회 및 표시 함수
//게시판 목록 버튼 눌렀을 때, 로그인 되었을 때
async function renderBoard() {
  //요청을 보내기 전에 AccessToken 빼오기
  const accessToken = localStorage.getItem("AccessToken");

  //만약에 로컬 스토리지에 AcessToken이 없으면 로그인 페이지로 전환
  if (!accessToken) {
    changePages(pageSignin);
    alert("로그인이 필요합니다.");
    return;
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/board/list`, {
      method: "GET",
      //fetch에서 headers안에 Authorization: `Bearer ${AcessToken}`
      headers: {
    ...
    if (responseData.status !== "success") {
      ...
      //요청에서 받아온 게시물들 forEach => ul안에 li로 넣기
...
async function editBoard(event) {
  ...
  if (!editTitleInput.value.trim() || !editContentInput.value.trim()) {
    alert("모든 항목을 입력해주세요");
    return;
  }

  const boardData = {
    boardId: editBtn.dataset.boardId,
...
//로그인 요청 함수
async function signinHandler(evnet) {
  event.preventDefault();
...
async function signupHandler(event) {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/auth/signup`, {
      ...
      body: JSON.stringify(signupData), //자바스크립트 객체를 json 문자열로 반환
    });
...
editBtn.addEventListener("click", () => {
  editTitleInput.value = detailTitle.innerText;
  editContentInput.value = detailContent.innerText;
